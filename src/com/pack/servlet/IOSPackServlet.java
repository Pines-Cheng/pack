...
public class IOSPackServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (command.equals("delete")) {
			...
			String url = packConfigDefine.IOS_PACK_HOST
	...
	private void pack(JSONObject js, String selectedCompanyName)
		...
		String url = packConfigDefine.IOS_PACK_HOST
				+ "/IOSPack?selectedCompanyName=" + selectedCompanyName
				+ "&date=" + date;
		String shellOperateLogPath = new String(packConfigDefine.IOS_PACK_HOST
				+ "/packTemp/" + date + "/shellOperateLog");
		// 打包临时文件夹的路径
		String IOSpackTempFileString = new String(
				packConfigDefine.IOS_PACK_TEMP_FILE + File.separator + date);

		if (!FileOperate
				.ifDirNotExistThenCreate(getCompanyName.tomcatCompanysFilePath
						+ File.separator + "zip" + File.separator + "IOS")) {
			js.put("status", "ERROR");
		...
		try {
			...
			js.put("status", "ERROR");
			js.put("message", "PC打包服务器未启动或者主机地址未正确配置！");
			return;
		}
